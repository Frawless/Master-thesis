...
\begin{itemize}
	...
	\item \textbf{PM initial}\,---\,Permgerm or Metaspace initial memory (either Permgen or Metaspace depending the JVM version).
	\item \textbf{PM committed}\,---\,Permgerm or Metaspace committed memory (either Permgen or Metaspace depending the JVM version).
	\item \textbf{PM max}\,---\,Permgerm or Metaspace maximum memory (either Permgen or Metaspace depending the JVM version).
	\item \textbf{PM used}\,---\,Permgerm or Metaspace used memory (either Permgen or Metaspace depending the JVM version).
...
\begin{description}
	...
	\end{itemize}	
	\emph{Inter-router} and \emph{route-container} modes are the same such as listener's modes.
\end{description}
...
multigraph: false	  
\end{verbatim}
From this metadata NetworkX create two nodes with type, id, and listener or connector attributes. This attributes will be used for generating configuration files for each node. All possible attributes that user can specify for each node is available in \ref{AP:Qpid-Dispatch Configuration File Template}.
...
\begin{minipage}{.4\textwidth}
  ...
  \captionof{figure}{Simple network with router agent.}
  \label{fig:agent_1}
\end{minipage}%
\begin{minipage}{.4\textwidth}
  \centering
  \includegraphics[width=.85\linewidth]{obrazky-figures/agent_2.pdf}
  \captionof{figure}{Simple network with router shut-down demonstration.}
  \label{fig:agent_2}
\end{minipage}
\end{figure}
...
\todo{After interconnect perf. demo}

\section{Collected Data Format and Evaluation}
\todo{Same as MPT, ask Otavio}
