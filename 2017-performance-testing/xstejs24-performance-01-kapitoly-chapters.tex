...
\begin{description}
	...
	\begin{itemize}
		\item \textbf{Java}\,---\,using for JMS-based testing, including \emph{AMQP} \cite{OASIS:AMQP}, OpenWire and Core protocols.
...
Qpid-Dispatch is a lightweight AMQP message router for building scalable available and performant messaging networks. With respect to ISO/OSI \footnote{\url{http://www.studytonight.com/computer-networks/complete-osi-model}} model, this router is an application layer program running as a normal user program or as a daemon. Key features of Qpid-Dispatch are:
...
To ensure security the router uses SSL/TLS protocol and related certificates and SASL protocol mechanisms to encrypt and authenticate remote peers. Router listeners act as network servers and connectors act as network clients. Both connection components may be configured securely with SSL/TLS and SASL.
...
Message may be delivered with various degrees of reliability such as 'at most once', 'at least once' and 'exactly once'.
...
Meta data - inventory and config file

\subsection{Topology Creation and Deployment}
networkX, graph site, ansible for deploy
